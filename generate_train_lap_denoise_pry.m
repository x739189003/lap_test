clear;
close all;
folder = '/home/zyy/jtt/T91-BSDS200';

savepath = '/home/zyy/jtt/1220-d6-lap.h5'
size_label = 128;
scale = 4;
...
data = zeros(size_label, size_label, 1, 1);
label_x2 = zeros(size_label, size_label, 1, 1);
label_x4 = zeros(0, 0, 1, 1);
                ...
                if size(image,3)==3
                    ...
                    for x = 1 + margain : stride : hei-size_label+1 - margain
                        for y = 1 + margain :stride : wid-size_label+1 - margain
                            ...
                            %%imshow(subim_label);
                            subim_label_nose = awgn(subim_label,20,'measured');
                            
                            %%subim_label=double(subim_label);
                            %%subim_label_nose = imnoise(subim_label,'gaussian',0,10^2/255^2);
                            
                         

                            count = count+1;
                            data(:, :, 1, count) = subim_label;
                            label_x2(:, :, 1, count) = subim_label_nose;
                            label_x4(:, :, 1, count) = 0;
                        end
                    end
                end
